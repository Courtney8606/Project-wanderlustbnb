...
def test_get_all_spaces(db_connection):
    ...
    assert repository.all() == [
        ...
        Space(1, 'Wizarding Cupboard', 'London', 50.00, 'A cosy room under the stairs. Comes with complementary spiders.', 1),
        Space(2, 'Amore Penthouse', 'Paris', 87.00, 'Within view of the Eiffel Tower, this penthouse is your parisian dream.', 2),
...
def test_create_space(db_connection):
    db_connection.seed("seeds/spaces_table.sql")
    repository = SpaceRepository(db_connection)
    space = Space(5, 'House', 'London', 75.50, 'This is a house', 3)
    repository.create(space)
    spaces = repository.all()
    assert spaces[-1] == Space(5, 'House', 'London', 75.50, 'This is a house', 3)
...
def test_update_space(db_connection):
    db_connection.seed("seeds/spaces_table.sql")
    repository = SpaceRepository(db_connection)
    space = Space(1, 'Cupboard under the stairs', 'London', 50.00, 'A cosy room under the stairs. Comes with complementary spiders.', 1)
    ...
    assert result == [
        ...
        Space(3, 'Paella Place', 'Madrid', 31.59, 'Eat paella and sleep.', 3),
        Space(4, 'Mi Casa', 'Madrid', 45.50, 'Es tu Casa.', 3),
        Space(1, 'Cupboard under the stairs', 'London', 50.00, 'A cosy room under the stairs. Comes with complementary spiders.', 1)
    ]
