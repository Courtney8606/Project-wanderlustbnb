...
def test_get_all_bookings(db_connection):
    ...
    assert repository.all() == [
        Booking(1, 4, datetime.date(2024, 7, 12), 1, 3, False, False),
        Booking(2, 3, datetime.date(2024, 7, 12), 2, 3, False, False),
        Booking(3, 2, datetime.date(2024, 7, 12), 1, 2, True, True)
    ]
...
def test_create_booking(db_connection):
    db_connection.seed("seeds/spaces_table.sql")
    repository = BookingRepository(db_connection)
    booking = Booking(None, 4, datetime.date(2024, 7, 12), 1, 3, False, False)
    repository.create(booking)
    Bookings = repository.all()
    assert Bookings[-1] == Booking(4, 4, datetime.date(2024, 7, 12), 1, 3, False, False)
...
def test_update_approval(db_connection):
    ...
    assert result == [
        Booking(1, 4, datetime.date(2024, 7, 12), 1, 3, True, True)
    ]
...
def test_view_marked(db_connection):
    ...
    assert repository.all() == [
        Booking(1, 4, datetime.date(2024, 7, 12), 1, 3, False, False),
        Booking(2, 3, datetime.date(2024, 7, 12), 2, 3, False, False),
        Booking(3, 2, datetime.date(2024, 7, 12), 1, 2, True, False)
    ]